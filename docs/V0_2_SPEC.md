...
indexed runner trace, and completion, fuel exhaustion, uncertified stops, and observation errors are
distinct. `runCapturingErrors` keeps the accepted prefix when a later process fails to spawn or
decode; the original `run` API continues to return `Except ObservationError`. The trace keeps
...
- `Cordis.DeepSeekCurlProviderAssemblyToolRound`, feeding that exact terminal assembly directly
  into dependent tool execution and certified assistant/tool-result session append without
  reparsing the completed body;
- `Cordis.DeepSeekCurlProviderAssemblyToolPrefix`, retaining a typed provider/tool prefix on
...
- `Cordis.DeepSeekSessionRequest`, which requires a `SourceAgreement` between an indexed
  ...
  exact raw `RequestPlan`, mode-indexed complete/streaming plans, an injected-transport complete
  execution retaining a dependent response certificate, and a local SSE process execution retaining
  wire frames; model/system/tool-schema metadata is explicit, while remote transport, provider
  controls, and parser-backed schema validity remain adapter obligations;
...
- `Cordis.DeepSeekHarnessLocalSseTimeout`, starting a loopback SSE fixture that flushes two valid
  ...
  external;
- `Cordis.DeepSeekHarnessLocalSseMultiTool`, starting a one-shot loopback SSE fixture that validates
  `stream: true`, emits the existing two-call terminal body, and feeds the actual curl response to
  `executeConversationMultiStreamRound`; both local call IDs, dependent executions, certified tool
...
- `Cordis.DeepSeekExternalGenericConversation`, providing a finite dependent script whose accepted
  dispatches form an indexed trace and whose completed, fuel-exhausted, uncertified, and captured
  observation-error outcomes are separated; `runCapturingErrors` preserves an accepted prefix when
...
assignments under the birth-erased paper relation. The conditional bisimulation adapter requires
separate detailed-tag and dependent-assignment transport laws.
...
dependent finite chain. Every link is connected to the previous package and derives its target
assignment; the terminal package retains reverse-oriented birth-erased relatedness and
...
49. `Cordis.DeepSeekHarnessEventFileStreamRetryCancellation` writes the supported current-Harness
    ...
    remain open.
    `Cordis.DeepSeekHarnessEventFileLocalSseRetryConversation` reuses that current-event file
    endpoint for two real loopback HTTP/SSE retry rounds, retaining both dependent request
    provenance certificates, one typed 503 per round, distinct rebuilt bodies, and exact
    `8 -> 10` session growth. File durability, provider authenticity, blocked-read interruption,
    external effects, and deployed Harness equivalence remain open.
    `Cordis.DeepSeekHarnessEventFileProcessSchema` reuses the same temporary-file event endpoint
    for the heterogeneous weather/clock schema process. Its dependent result retains source/read
...
`Cordis.DeepSeekProcessScopedRequestConversation` closes the request-provenance gap around that
conversation. It rebuilds a typed `stream = true` request from the current session, passes that
request to the process/SSE adapter, and stores the successful scoped round under the same exact
