...
| Conditional confluence kernel    | `GlobalPaperTraceConfluence` proves, for any decreasing system with local joinability, a global join and uniqueness of irreducible endpoints; the Boolean witness exercises two branch orders.                                                         | Generic Newman-style metatheory only. CORDIS local diamonds, measure, strategy, canonical form, and Theorem 73 remain supplied/open.                                                                                                   |
...
normalizer: local joinability plus a strictly decreasing measure imply global joins and unique
irreducible endpoints. This closes only that generic proof kernel; the CORDIS-specific local
diamonds, measure, strategy, canonical form, and Theorem 73 remain unproved.
