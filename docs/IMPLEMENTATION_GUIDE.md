...
| Conditional confluence kernel    | `GlobalPaperTraceConfluence` proves global joins and unique irreducible endpoints; `AuthorityLinked` and `normalize_results_unique` apply it to actual `TracePackage` normalizer results, while the Boolean witness exercises two branch orders.       | Conditional Newman-style metatheory only. CORDIS local joins, measure, strategy, canonical form, and Theorem 73 remain supplied/open.                                                                                                  |
...
| Finite progress runner           | `ProgressAuthority` and `StepPotential` drive an exact dependent `runFuel`; endpoint well-formedness, lifecycle trace length, and quiescent/full-fuel stop certificates are retained, and initial-potential funding proves quiescent completion.       | Conditional finite execution bridge only; authorities and potential are supplied. No derived freshness, target-turn accounting, fairness, maximal termination, trace-wide assignment, support, confluence, or unrestricted Theorem 66. |
| Progress trace assignment        | `GlobalProgressAssignment` extends the runner authority with one `StepProgramAssignment` per lifecycle transition and recursively reconstructs a dependent `TraceProgramAssignment` for the complete finite trace, preserving the runner certificates. | Provenance is explicit input: no inference of fixed programs, roots, oracles, or reachability from raw transitions, and no full Definition 60/66 provenance theorem.                                                                   |
...
turn/step/tool subset plus text/reasoning/image user/assistant surface blocks and complete assistant
...
refinement state; text enters the smaller local message types and reasoning/image blocks remain wire-visible.
Both start
...
text-only user-message, text/reasoning/complete-tool-call assistant-message, and singleton-text
tool-result subset. It encodes canonical ASTs and composes them with the existing JSONL text and
UTF-8 byte parser; its round-trip theorems retain safe integers, assistant usage, source references,
replacement ranges, call IDs, raw tool-call arguments, and exact `isError` values. Keep assistant
image blocks, opaque provider metadata, unsupported surface operations, and unsupported event
variants outside this encoder rather than assigning them guessed semantics.
...
semantic payload types beyond raw retention for assistant image blocks, provider usage/failure objects, tool-result `error`/
...
positions, and returns a typed required-opaque error otherwise. The adjacent
`Cordis.DeepSeekHarnessEventIgnorableNormalization` module continues only the supported subset: it
renumbers retained rows contiguously, remaps supported `sourceEventSeqs`/`surfaceOp` references,
and validates the rewritten JSON as a local session. Duplicate physical sequences, missing
...
cancellation stops return explicit nonterminal prefix errors, so this layer does not turn a partial
process read into a fabricated Harness completion or claim deployed async behavior.
...
`Cordis.DeepSeekHarnessEventPrefix` is the pure stateful target immediately below that text path.
`Cursor.push` consumes one already-framed JSON object, decodes it, refines it against the current
`SessionRefinement.State`, and appends a dependent `PrefixSequence` node carrying the exact
protocol delta. `run` adds a finite fuel bound and a cooperative before-entry stop with an explicit
unread suffix. Keep framing, blocked-read interruption, crash durability, and deployed equivalence
as separate later contracts; this module is deliberately IO-free.
...
`GlobalPaperTraceConfluence` now supplies the reusable conditional metatheory above the finite
normalizer: `AuthorityLinked` reconstructs the selected finite path from `normalizeFuel`, and
`normalize_results_unique` proves that two successful normalizer results have equal final
`TracePackage` endpoints whenever a `ConfluentAuthority` supplies local joinability. This closes
only that conditional bridge; the CORDIS-specific local joins, measure, strategy, canonical form,
and Theorem 73 remain unproved.
...
maximal termination.
`GlobalProgressRun` is the finite execution bridge above those authorities: a supplied
`ProgressAuthority` and strict `StepPotential` drive `runFuel`, which preserves endpoint
well-formedness and returns an exact lifecycle trace with either quiescence or full-fuel exhaustion.
Initial-potential funding proves quiescence, but the authorities, target-turn accounting,
freshness, fairness, trace-wide assignment, maximal termination, support, and confluence remain
explicitly outside the claim. `GlobalSupport` refutes the printed combined-order inference, then proves
`GlobalProgressAssignment` supplies the separate provenance bridge: its authority assigns every
lifecycle transition to a `StepProgramAssignment`, and `assignTrace` reconstructs the dependent
assignment for the complete intrinsic trace. This preserves the runner certificates but does not
infer fixed programs, roots, oracles, or reachability from raw transitions.
