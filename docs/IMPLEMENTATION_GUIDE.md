...
text-only user-message, text/reasoning assistant-message, and singleton-text tool-result subset. It
encodes canonical ASTs and composes them with the existing JSONL text and UTF-8 byte parser; its
round-trip theorems retain safe integers, assistant usage, source references, replacement ranges,
call IDs, and exact `isError` values. Keep assistant image/tool-call blocks, opaque provider
metadata, unsupported surface operations, and unsupported event variants outside this encoder
rather than assigning them guessed semantics.
