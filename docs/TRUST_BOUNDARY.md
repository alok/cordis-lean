...
and time values, accepts selected `request/header` snapshots and text-only index-zero
`assistant/chunk` records, normalizes one-based upstream steps to zero-based local steps, derives
...
replay state, reasoning and multimodal blocks,
...
| `SessionRefinement.validateJsonLog`                                | Stateful supported-subset decoding returns exact rich append/replacement witnesses, selected request-header and text-only assistant-chunk log records, text/complete-assistant-tool-call surface metadata in `State.wireSurface`, runtime protocol witnesses when applicable, fresh local call-ID evidence reused by call/result events, and a cumulative projection theorem. | No complete event union, unsupported header/chunk shapes, unsupported/malformed replacement shapes, persisted JSONL parser, timestamp truth, crash recovery, or whole-session equivalence.                                                                                                                                       |
