...
`RuntimeOutcomeSession.validateAndDispatch` is the adjacent pure runner bridge: success appends a
finished assistant view, while normalized failure returns the exact failure certificate and
leaves the runner unchanged. It does not fabricate failure messages or session events.
...
| `RuntimeOutcomeSession.validateAndDispatch` | **Checked/proved:** successful validated JSON appends a finished assistant view through the existing pure runner; normalized failure returns its exact certificate with the runner unchanged. | No fabricated failure message, retry/cancellation policy, source-event synthesis, persistence, provider authenticity, or whole-runtime equivalence. |
