...
| `DeepSeekStreamHarness.executeConversationStreamRound`, `ConversationRunner.appendFinished`                                                                                                               | **Checked/exercised at the complete-body stream/harness boundary:** a terminal rich tool stream retains its wire/session certificate, assigns local numeric IDs, routes each streamed call through dependent admission/policy/provider execution, and appends certified typed results to the reusable conversation runner; a deterministic process fixture exercises the path.                                                                                                                                                                                                                                                                                                           | No incremental delivery, cancellation, backpressure, reconnect, provider-complete assembly, credential/network/process-trust, or deployed Harness equivalence is proved.                                                                                                                                                         |
...
- `SessionRefinement` accepts the complete Harness event union, preserves every source field in
  ...
  them payload semantics or replay behavior;
