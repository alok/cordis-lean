...
| `SessionRefinement.SurfaceCodec.encodeWireEvent`, `decode_encode`, `decodeWireEventsText_of_encoded`, `decodeWireEventsBytes_of_encoded`                                                                  | **Checked/proved/exercised:** reverse codec covers text-only user, text/reasoning/complete-tool-call assistant, and singleton-text tool events; AST/text/bytes retain safe integers, usage, provenance, ranges, IDs, raw tool-call arguments, and `isError`, with assistant/user/tool and invalid-UTF-8 fixtures.                                                                                                                                                                                                                                                                                                                                                                                           | Narrow source-shaped local serialization boundary layered over `SessionRefinement`.                                                                                                                                                                                                                                                                    | No image encoder, opaque metadata encoder, unsupported surface operations, full event-union codec, deployed logger equivalence, provider obedience, or persistence theorem is claimed.                                      |
