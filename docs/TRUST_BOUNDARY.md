...
to fresh numeric IDs with proof-carrying uniqueness state. Text-only `user/message` and
`assistant/message` events additionally retain source IDs, provider/model metadata, usage, and
provenance references in `State.wireSurface`, while projecting only concatenated text into the
smaller local `Session.Message` vocabulary. Each admitted event carries both a
...
Chunks, headers, replacements, extensions, opaque tool metadata, replay state, multimodal blocks,
...
| `SessionRefinement.validateJsonLog`                                | Stateful supported-subset decoding returns exact rich appends, text-only surface metadata in `State.wireSurface`, runtime protocol witnesses when applicable, fresh local call-ID evidence, and a cumulative projection theorem.                                                                                                       | No complete event union, replacement translation, persisted JSONL parser, timestamp truth, crash recovery, or whole-session equivalence.                                                                                                                                                                                         |
...
| `DeepSeekSessionRunner.finishText`, `finishTool`, `Runner.append`  | Composes accepted text/one-tool terminal responses into a pure append-only session runner with exact sequence, message-order, and tool-count invariants.                                                                                                                                                                               | No transport, cancellation, persistence, external execution, or whole-session equivalence.                                                                                                                                                                                                                                       |
...
- `DeepSeekApi` proves live HTTP delivery, credential validity, complete provider schema
  compatibility, streaming behavior, or local `ToolSpec` validation; it is a typed codec plus
  an explicit transport seam exercised by a deterministic fixture;
...
- `DeepSeekSessionRunner` proves no live transport, cancellation, persistence, external tool
  ...
  sequential composition whose local IDs are allocated by an explicit tool-call count;
...
- `SessionRefinement` accepts the complete Harness event union, preserves every source field in
  ...
  text-only surface subset retains selected source metadata in `State.wireSurface` but still
  intentionally projects into smaller local message types;
