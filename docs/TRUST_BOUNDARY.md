...
`DeepSeekStreamHarnessRetryConversation` is the process-backed streamed analogue. Its
`runWithFinish` entry point carries a caller-supplied finisher through an indexed trace that
retains each complete SSE body, ordered retry history, dependent assistant/tool endpoint, and
exact runner/model tail; `run` remains the multi-tool wrapper. Direct fixtures cover text/tool/
mixed/multi semantic raw counts, while the dependent loop covers text plus the admitted counter
tool. Backoff, idempotency, blocked-read cancellation, persistence, reconnects, external effects,
...
`runWithFinish` carries a caller-supplied text/tool/mixed/multi finisher, while `run` remains the
multi-tool wrapper. The cancellation constructor retains the exact accepted retry-aware prefix,
unchanged runner/model endpoint, round/reason decision, and retry history inside each retained head;
direct text completion and tool cancellation fixtures exercise both paths. It does not interrupt
...
`DeepSeekHarnessEventLocalSse` then sends that same indexed endpoint through the existing real
loopback Python/curl SSE fixture. `EventRunner` makes the validated session the runner index and
`EventSseResult` retains the request plan, raw frames, finisher, and exact append endpoint; the
fixture checks one valid request, three frames, and `nextSeq 6 -> 7`. This is local process/HTTP
...
Harness equivalence.
`DeepSeekHarnessCompleteSimulation` composes the archive, supported-subset normalization, and
occurrence-indexed replay into one dependent result. Its executable text, tool-message, and
compaction-style replacement fixtures expose archive/known/keep/drop, endpoint, source-position,
...
| `DeepSeekSchemaTrace.SchemaConversationTrace`, `rounds`, `rounds_length`, `last_round_is_endpoint`                                                                                                             | **Checked/proved:** the dependent history indexes every successful tool round by its exact starting and ending runner/model, proves its finite list projection and length, and exposes the final round endpoint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Intrinsic local history certificate for the schema conversation loop.                                                                                                                                                                                                                                                                                   | No provider obedience, retries, cancellation, persistence, external effects, or deployed Harness equivalence is proved by this history certificate.                                                                                |
| `DeepSeekSchemaConversationLoop.executeSchemaRegistryConversationStep`, `runSchemaConversation`                                                                                                           | **Checked/proved/exercised:** a validated no-tool response is retained as a terminal witness; certified tool rounds advance the dependent model and runner under explicit fuel; the intrinsic trace equals its legacy list projection, and exhaustion is distinct.                                                                                                                                                                                                                                                                                                                                                                                                                                         | Caller-fueled transport-backed heterogeneous conversation loop.                                                                                                                                                                                                                                                                                        | Complete-body and caller-fueled only; no retries, in-flight cancellation, persistence, external effects, provider obedience, or deployed Harness equivalence is proved.                                                                    |
...
| `DeepSeekHarnessEventLocalSse.EventRunner`, `EventSseResult`, `Example.run`                                                                                                                                    | **Checked/proved/exercised local transport continuation:** the validated event endpoint is the exact `ExtensionRunner` session index; the real loopback Python/curl SSE request, frames, finisher, protocol replay, and append endpoint are retained, with the header fixture checking one valid request and `nextSeq 6 -> 7`.                                                                                                                                                                                                                                                                                                                                                                  | Validated current-Harness JSON endpoint → local indexed SSE round.                                                                                                                                                                                                                                                                                | Local process/HTTP evidence only; no remote/provider authenticity, credentials, persistence, cancellation delivery, or deployed-Harness equivalence claim.                                                                               |
...
| `DeepSeekHarnessCompleteSimulation.CompleteSimulation`, `simulate`, `textSummary`, `toolSummary`, `replacementSummary`                                                                                    | **Checked/proved/exercised complete supported-subset composition:** archive, normalization, and occurrence-indexed replay share one dependent result; three source-shaped fixtures expose executable archive/known/keep/drop, endpoint, source-position, and source-time summaries, while malformed replacement rejects.                                                                                                                                                                                                                                                                                                                                                                                    | Source-shaped current-Harness JSON AST → certified local archive/session replay.                                                                                                                                                                                                                                                                       | Finite source-honest subset only; opaque/provider/transport/persistence/cancellation semantics and complete deployed-Harness equivalence remain external.                                                                                  |
...
| `DeepSeekStreamHarnessRetry.RetryPolicy`, `RetryHistory`, `executeWithRetryAndFinish`, `executeConversationStreamRound`, `executeWithRetry`, `executeConversationMultiStreamRound`                        | **Checked/exercised streamed retry path:** caller-supplied text/tool/mixed/multi finishers are accepted at the complete-body and dependent-round boundaries; process and transient-HTTP failures are retried only under an explicit policy; exact ordered `SessionClientError` history carries a retry bound, and a successful round retains the existing streamed wire/session/tool certificates. Stream framing, semantic response, and tool failures are terminal.                                                                                                                                                                                                                                       | Complete-body immediate retry over the streamed process boundary, with multi-tool convenience wrappers.                                                                                                                                                                                                                                                | Provider backoff, idempotency of arbitrary tools, cancellation, persistence, asynchronous delivery, and deployed Harness retry equivalence remain outside.                                                                                 |
...
| `DeepSeekStreamHarnessRetryCancellation.RetryCancellableStop`, `RetryCancellableRunResult`, `runWithFinish`, `runAux`, `run`                                                                              | **Checked/exercised retry-aware pre-round cancellation path:** `runWithFinish` accepts caller-supplied text/tool/mixed/multi finishers and `run` preserves the multi-tool wrapper; cancellation is checked before each process-backed retry round, and the exact accepted streamed retry prefix, unchanged runner/model endpoint, decision certificate, and per-head retry histories are retained, while completion and fuel exhaustion remain distinct.                                                                                                                                                                                                                                                    | Composition of the indexed streamed retry boundary with caller-controlled cancellation.                                                                                                                                                                                                                                                                | No interruption of in-flight process/HTTP/stream/tool IO; no cleanup, backpressure, persistence, reconnect, asynchronous delivery, or deployed Harness cancellation equivalence.                                                           |
...
| `GlobalRuleRelatedTraceReplay`                                                                                                                                                                             | Adds an exact all-keep `RuleRelated` replay API. Each local match carries exact rule/actor equations, endpoint well-formedness, successor relation, and dependent assignment transport; recursive replay derives final exact relatedness and complete shadow assignment, with an executable six-step diagonal witness.                                                                                                                                    | Exact relation replay from supplied local matches.                                                                                                                                                                                                                                         | The theorem does not prove arbitrary dynamics respect `RuleRelated`, erase allocator control, or establish global lifecycle simulation, Lemma 72, or Theorem 73.                                                                                       |
| `GlobalRuleRelatedBisimulationReplay`                                                                                                                                                                      | Adapts the conditional well-formed unified-rule bisimulation to exact replay. Its `BisimulationAssignmentTransport` field explicitly supplies dependent assignment transport, after which the existing all-keep induction derives the exact shadow trace and endpoint relation.                                                                                                                                        | Conditional bisimulation-to-replay adapter.                                                                                                                                                                                                                                                | Assignment transport is not derived from rule/actor equality or base dynamics; no allocator erasure, episode deletion, global lifecycle theorem, Lemma 72, or Theorem 73 is claimed.                                                         |
| `GlobalRuleRelatedDetailedReplay`                                                                                                                                                                          | Adds a constructor-sensitive `detailedRule` obligation to exact `RuleRelated` matches, packages both forward and backward all-keep traces as birth-erased paper replays, and adapts a conditional bisimulation only with explicit detailed-tag and assignment transport.                                                                                                                                                                                                                                   | Detailed-tag exact replay projection in both orientations.                                                                                                                                                                                                                                                      | The detailed-tag field is not inferred from coarse global rule equality; no lifecycle simulation, allocator erasure, episode deletion, Lemma 72, or Theorem 73 is claimed.                                                               |
| `GlobalPaperLandingReplay`                                                                                                                                                                                | Adds a concrete fixed-program Iter/Finish landing diamond to the assigned trace layer; actual activation witnesses reconstruct both assignments, and the dependent rewrite proves reflexive birth-erased endpoint relatedness, endpoint well-formedness, and swapped detailed rule/actor ledgers.                                                                                                                                                                                                                                                                                                                                                                                                           | Occurrence-specific landing certificate.                                                                                                                                                                                                                                                                                                               | It does not provide global lifecycle simulation, general episode deletion, normalization, Lemma 72, or Theorem 73.                                                                                                                         |
| `GlobalPaperFullLifecycleReplay`                                                                                                                                                                          | Packages the existing heterogeneous example as one exact six-record dependent trace (`L-Begin`, `L-Iter`, `L-Finish`, `O-Retire`, `L-Leave`, `L-Unload`), with actual activation assignments for the first three records, explicit non-activation assignments for the rest, and an all-keep self-replay retaining endpoint/WF, detailed rules, actors, decisions, and the complete assignment.                                                                                                                                                                                                                                                                                                              | Concrete full-path proof certificate over one existing fixture.                                                                                                                                                                                                                                                                                        | It does not infer arbitrary program provenance, match an unrelated peer lifecycle, provide global lifecycle bisimulation, delete a general episode, or prove Lemma 72/Theorem 73.                                                          |
| `GlobalPaperShiftedLifecycle`                                                                                                                                                                             | Supplies a concrete non-reflexive replay with only the allocator clock shifted: dependent L-Leave/L-Unload transitions and their assigned trace are rebuilt at the peer, with exact rule/actor, well-formedness, and birth-erased endpoint certificates.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Trace-local allocator-clock witness.                                                                                                                                                                                                                                                                                                                   | It does not prove global lifecycle matching or remove the independent clock-sensitive unmatched-transition countermodel.                                                                                                                   |
| `GlobalPaperShiftedFullLifecycle`                                                                                                                                                                         | Extends the allocator-clock peer to the complete assigned six-step path: concrete L-Begin/L-Iter/L-Finish/O-Retire/L-Leave/L-Unload transitions and assignments are rebuilt, and the relation-aware replay retains endpoint/WF, final relatedness, exact detailed rules, actors, six keep decisions, and the explicit peer assignment.                                                                                                                                                                                                                                                                                                                                                                      | Non-reflexive full-path replay certificate.                                                                                                                                                                                                                                                                                                            | It is one finite occurrence-indexed witness, not global lifecycle bisimulation, arbitrary provenance inference, episode deletion, Lemma 72, or Theorem 73.                                                                                 |
| `GlobalPaperNonReflexiveRewrite`                                                                                                                                                                          | Instantiates a related-endpoint adjacent rewrite with opposite-order inserts: the normal and swapped endpoints are distinct but birth-erased-related, a trailing O-Retire is replayed from the related endpoint, and the result carries WF, assignment, and rule/actor permutation certificates.                                                                                                                                                                                                                                                                                                                                                                                                            | Concrete non-reflexive relation-aware rewrite witness.                                                                                                                                                                                                                                                                                                 | It remains occurrence-specific and orchestration-suffix-only; no global lifecycle bisimulation, normalization, Lemma 72, or Theorem 73 is derived.                                                                                         |
| `GlobalPaperTraceBackwardRewrite`                                                                                                                                                                         | Mirrors the related-endpoint rewrite backward: it replays the source suffix from the related swapped endpoint, transports the dependent assignment, and retains endpoint/WF plus rule/actor permutation certificates; the concrete birth-gap witness exposes actors `[2, 1, 1]`.                                                                                                                                                                                                                                                                                                                    | Backward finite relation-aware rewrite witness.                                                                                                                                                                                                                                                                                                       | It remains occurrence-specific and orchestration-suffix-only; no symmetric lifecycle simulation, normalization, Lemma 72, or Theorem 73 is derived.                                                                                   |
| `GlobalPaperTraceBackwardNormalization`                                                                                                                                                                   | Composes backward related-rewrite certificates through dependent trace packages, reconstructing each target assignment and retaining reverse-oriented endpoint relatedness plus rule/actor permutations.                                                                                                                                                                                                                                                                                                                                                   | Backward finite certificate composition.                                                                                                                                                                                                                                                                                                            | It supplies no rewrite strategy, canonical form, termination, confluence, Lemma 72, or Theorem 73.                                                                                                                               |
| `GlobalPaperTraceBidirectionalNormalization`                                                                                                                                                               | Unifies forward and backward related-rewrite certificates behind one orientation-indexed chain; target packages, assignments, and permutation facts are derived from the selected certificate, with backward relation symmetrized only at the wrapper boundary.                                                                                                                                                                                                                                                                                       | Bidirectional finite certificate composition.                                                                                                                                                                                                                                                                                                       | It supplies no rewrite strategy, canonical form, termination, confluence, Lemma 72, or Theorem 73.                                                                                                                               |
| `GlobalPaperTraceBidirectionalNormalizer`                                                                                                                                                                  | Adds a conditional fuel-bounded normalizer over the unified chain: an explicit authority supplies normal-form decisions, oriented links, and a strictly decreasing Nat measure, and the resulting terminal package retains birth-erased relatedness plus rule/actor permutations.                                                                                                                                                                                                                                                                       | Conditional normalizer above bidirectional certificates.                                                                                                                                                                                                                                                                                           | It does not infer a strategy, measure, dynamic termination, canonical form, confluence, Lemma 72, or Theorem 73 from CORDIS dynamics.                                                                                               |
| `GlobalPaperTraceBidirectionalConfluence`                                                                                                                                                                  | Lifts the decreasing/local-join Newman kernel to the bidirectional authority: selected links become explicit rewrite edges, and supplied local joins, strict measure decrease, and normal-form equivalence prove unique terminal packages for successful normalizer runs.                                                                                                                                                                                                                                                                            | Conditional confluence above bidirectional certificates.                                                                                                                                                                                                                                                                                            | It does not derive local joins, a strategy, a measure, CORDIS-specific confluence, Lemma 72, or Theorem 73.                                                                                                                     |
...
| `GlobalNamePaperRelation`                                                                                                                                                                                 | Adds `ValueActionEquiv` as an explicit per-key setoid-invariance premise; `contextRelated_actTable` transports active-context observations, and renamed paper relations compose/invert with `nameActionPaperRelated_trans`/`nameActionPaperRelated_symm`, including acted-state corollaries.                                                                                                                                                                                                                                                                                                                                                              | Closure laws for the observational name-action bridge.                                                                                                                                                                                                                                                                                                                                                         | These are observational relation laws, not unconditional lifecycle bisimulation; activation transport, dynamics, oracle, recovery, and inertia premises remain explicit.                                                                                                                     |
...
deployment or upstream interoperability. `Cordis/AxiomAudit.lean` runs `#print axioms` for 2689
...
- `DeepSeekCurlIncrementalOutcome` adds only the typed post-validation composition: provider
  failures retain the exact failure witness and unchanged local runner, while successful rich
  outcomes append. It proves no byte-level framing, backpressure, cancellation, reconnect,
  network/credential/process trust, provider-complete assembly, or deployed equivalence;
- `DeepSeekHarnessLocalSseOutcome` carries the same typed outcome through the real loopback SSE
  boundary. The result retains the prepared streaming request, line observations, reconstructed body,
  strict wire/outcome certificates, dispatch endpoint, request report, and clean server exit;
...
- `DeepSeekHarnessLocalSseApiErrorRetry` composes that boundary with one real loopback retry: the
  first valid request is retained as a typed 429/API-error envelope, the second yields strict SSE
