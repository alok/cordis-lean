...
retains the successful `buildChatRequestFor` equation before constructing a `RequestPlan`.
Optional DeepSeek controls are caller policy. The module does not prove parser-backed schema
validity, provider obedience, credentials, transport success, or deployed equivalence.
