...
| `DeepSeekOutcomeConversation.dispatchOutcome`, `executeAndDispatchOutcome`, `projectedFunctionCalls` | **Checked/proved/exercised:** a process-backed provider failure preserves `ConversationRunner`; a successful rich terminal view appends with model/tool-count certificates and exposes its completed provider calls as ordinary `FunctionCall` values. Deterministic failure/text/tool/mixed/multi fixtures cover the branches. | Typed terminal-outcome handoff into the existing dependent conversation executor. | The bridge does not execute tools, authenticate provider IDs, choose failure/retry policy, persist, cancel, or prove whole-session/deployed equivalence. |
