...
| `SessionRefinement.SurfaceCodec.encodeWireEvent`, `decode_encode`, `decodeWireEventsText_of_encoded`, `decodeWireEventsBytes_of_encoded`                                                                  | **Checked/proved/exercised:** the canonical reverse codec covers text-only user messages and singleton-text tool results. AST encoding, JSONL text, and UTF-8 bytes retain safe integers, provenance, replacement ranges, call IDs, and exact `isError`; the executable fixture covers a user append, a tool replacement, and invalid UTF-8 rejection.                                                                                                                                                                                                                                                                                                                                                      | A deliberately narrow source-shaped local serialization boundary layered over `SessionRefinement`.                                                                                                                                                                                                                                                     | No assistant-message encoder, image/opaque metadata encoder, full event-union codec, deployed logger equivalence, provider obedience, or persistence/durability theorem is claimed.                                         |
