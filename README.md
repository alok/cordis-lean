...
`Cordis.DeepSeekHarnessPayloadPersistenceStreamRetryCancellation` carries that same payload ledger
through a certified pre-round cancellation prefix. The fixture accepts the first process-backed
tool round, retains all eight restored payload rows, then stops before selecting the next request;
its executable projections check `8 -> 11`, one typed round, two first-round calls, timeout, and
model `0`. This is a typed local cancellation boundary, not interruption of a blocked read or
HTTP request, process cleanup, durable recovery, external effects, or deployed equivalence.

`Cordis.DeepSeekHarnessPayloadPersistenceFileStreamRetryCancellation` repeats that composition
through an actual temporary-file `DurableIO.FileBackend`. The dependent result retains exact
source/read-byte equality, the payload ledger, restored session, and the same typed cancellation
prefix; its fixture checks equal file bytes, eight payload rows, `8 -> 11`, one round, timeout, and
model `0`. The temporary-file lifetime does not prove fsync, stable media, crash recovery,
blocked-read interruption, cleanup, external effects, or deployed equivalence.
...
exact `ExtensionRunner.appendFinished` endpoint; the executable header fixture checks one valid
request, three frames, and `nextSeq 6 -> 7`. This is local process/HTTP evidence only, not remote
provider, credential, persistence, cancellation, or deployed-Harness equivalence.
...
`Cordis.DeepSeekHarnessCompleteSimulation` composes the lossless archive, supported-subset
normalization, and occurrence-indexed replay into one `CompleteSimulation` result. Its executable
text, tool-message, and compaction-style replacement fixtures expose exact archive/known/keep/drop,
...
`Cordis.DeepSeekHarnessCompleteRequest` is the next dependent composition: `prepare` runs that
complete simulation and then prepares a `PreparedLogRequest` from the exact normalized validated
endpoint. The result carries the archive, keep/drop ledger, normalized replay, request/header
agreement, request-build equality, and a proof that the request log is the validator's endpoint.
The executable seven-row ignorable-header fixture succeeds, while a required opaque row rejects
before request preparation. The indexed decoder/replay certificate now proves that the replay
endpoint equals the validator endpoint; opaque semantics, provider behavior, transport,
persistence, cancellation, and complete deployed Harness equivalence remain external.
...
runner endpoint for one round. `Cordis.DeepSeekSchemaTrace` gives the loop an intrinsic dependent
history: every certified tool round is indexed by its starting runner/model and its exact produced
runner/model endpoint, with a list projection theorem for compatibility. `Cordis.DeepSeekSchemaConversationLoop`
then distinguishes a certified no-tool terminal response from fuel exhaustion while carrying that
trace and its finite list projection. The loop is still caller-fueled and complete-body; it does not claim
...
`Cordis.GlobalRuleRelatedTraceReplay` adds the exact-relation companion. Its local forward and
backward matches carry exact rule/actor equations, endpoint well-formedness, successor
`RuleRelated`, and dependent assignment transport. The all-keep induction constructs the shadow
trace and assignment, proving final exact relatedness and ledger projections; the diagonal
six-step fixture is executable. Non-reflexive lifecycle matching remains supplied evidence.

`Cordis.GlobalRuleRelatedBisimulationReplay` adapts the conditional well-formed ten-rule
bisimulation to this exact replay surface. `BisimulationAssignmentTransport` keeps the missing
dependent assignment law explicit; once supplied, the adapter constructs local exact matches and
reuses the all-keep induction. It does not infer assignment transport from rule or actor equality.

`Cordis.GlobalRuleRelatedDetailedReplay` adds the constructor-sensitive `detailedRule` proof
...
rewritten through `RelatedAdjacentRewrite`, and checked for the reflexive birth-erased endpoint,
well-formedness, swapped detailed rules, swapped actors, and transported assignment. It remains an
occurrence-specific landing certificate; it does not provide global lifecycle simulation or close
Lemmas 71/72 or Theorem 73.

`Cordis.GlobalPaperFullLifecycleReplay` packages the existing heterogeneous fixture as one complete
six-step dependent path: `L-Begin`, `L-Iter`, `L-Finish`, `O-Retire`, `L-Leave`, and `L-Unload`.
Begin/Iter/Finish carry actual fixed-program activation assignments; Retire/Leave/Unload carry
explicit non-activation assignments. An all-keep `DeletionReplay` replays that exact path against
itself and retains its endpoint, well-formedness, detailed rules, actors, six decisions, and full
assignment. This is a concrete full-path certificate, not peer-state lifecycle bisimulation,
arbitrary program-provenance inference, Lemma 72, or Theorem 73.
...
`Cordis.GlobalPaperShiftedFullLifecycle` applies the same allocator-only peer construction to
the complete six-step fixture. It reconstructs assigned peer transitions for Begin, Iter,
Finish, Retire, Leave, and Unload, then uses the relation-aware assigned trace replay to retain
the shifted endpoint's well-formedness, final birth-erased relatedness, exact six-rule and
six-actor ledgers, six keep decisions, and equality with the explicitly built peer assignment.
This is a finite non-reflexive replay certificate, not global lifecycle bisimulation or a proof
of Lemma 72/Theorem 73.

`Cordis.GlobalPaperNonReflexiveRewrite` supplies the first concrete non-reflexive related-window
rewrite: opposite-order inserts share a predecessor but end at distinct birth-erased-related
states, and a trailing `O-Retire` is replayed from the swapped endpoint. The witness carries
endpoint well-formedness, a transported assignment, and rule/actor permutations; it remains
occurrence-specific and does not claim global lifecycle simulation or normalization.

`Cordis.GlobalPaperTraceBackwardRewrite` mirrors that occurrence-specific construction in the
backward orientation. It replays a source suffix from the related swapped endpoint, transports
the dependent assignment, and retains endpoint well-formedness plus rule/actor permutations;
its concrete birth-gap witness exposes the reversed actor ledger `[2, 1, 1]`. This is still a
finite relation-aware rewrite certificate, not a symmetric lifecycle simulator or a proof of
Lemma 72/Theorem 73.

`Cordis.GlobalPaperTraceBackwardNormalization` composes those backward certificates into a
dependent finite chain. Each link is connected to the previous package, derives its target
assignment from the transported occurrence assignment, and composes the reverse-oriented final
relation and rule/actor permutations. It remains certificate composition only: no strategy,
canonical form, termination, confluence, Lemma 72, or Theorem 73 is inferred.

`Cordis.GlobalPaperTraceBidirectionalNormalization` unifies the forward and backward witnesses
behind one orientation-indexed chain API. The wrapper symmetrizes only the backward endpoint
relation and derives the target package, assignment, and permutation facts from the selected
certificate. Concrete forward and backward fixtures share the same birth-gap state family; the
API remains a finite supplied chain rather than an automatic normalizer.

`Cordis.GlobalPaperTraceBidirectionalNormalizer` adds a conditional fuel-bounded normalizer over
that unified chain. Its `Authority` supplies normal-form decisions, a source-indexed oriented link,
and a strictly decreasing natural measure; `normalize_some` constructs a finite terminal chain
retaining birth-erased relatedness and rule/actor permutations. It does not infer a strategy,
canonical form, dynamic termination, confluence, Lemma 72, or Theorem 73.

`Cordis.GlobalPaperTraceBidirectionalConfluence` lifts the decreasing/local-join Newman kernel to
that orientation-indexed authority. A `ConfluentAuthority` makes the selected bidirectional link
an explicit rewrite edge, supplies local joinability and normal-form equivalence, and yields unique
terminal packages for any two successful normalizer results. It remains conditional: CORDIS does
not derive the local joins, strategy, measure, confluence, Lemma 72, or Theorem 73.
...
| Complete current-Harness event fixtures compose archive, normalization, and source replay into one dependent result                                                                                                                                         | `DeepSeekHarnessCompleteSimulation.CompleteSimulation`, `simulate`, `textSummary`, `toolSummary`, `replacementSummary`, `malformedReplacementRejected`, `archive_raw_exact`, `protocolTrace_erase`, `sessionProjection_eq`                                                                                                                                                                                    | Three source-shaped supported fixtures expose executable archive/known/keep/drop, endpoint, source-position, and source-time summaries; malformed replacement rejects. This remains a finite source-honest subset bridge, not opaque/provider/transport/persistence/cancellation or complete deployed-Harness equivalence.                                                                                                                                                                                                                               |
| Complete current-Harness simulation composes into a certified request without losing its dependent archive/replay evidence                                                                                                                                  | `DeepSeekHarnessCompleteRequest.PreparedSimulation`, `prepare`, `prepareSimulation`, `PreparedSimulation.replay_endpoint_eq_validated`, `ignorableHeaderPrepared`, `requiredOpaqueRejected`                                                                                                                                                                                                                      | The request certificate is indexed by the exact normalized validator endpoint and retains the archive, keep/drop ledger, semantic replay, source/header agreement, request-build equality, and replay-endpoint equality. The positive ignorable-header fixture and required-opaque rejection are executable. |
...
| `Cordis.GlobalRuleRelatedTraceReplay`                           | Exact all-keep `RuleRelated` replay from supplied local step matches, preserving endpoint well-formedness, exact rules/actors, and dependent `TraceProgramAssignment` transport; its diagonal six-step fixture is executable, while non-reflexive lifecycle matching remains an explicit certificate.                                                                                                                                                                                                                                                                               |
| `Cordis.GlobalRuleRelatedBisimulationReplay`                    | Adapter from the conditional well-formed ten-rule bisimulation to exact replay. It derives the exact actor wrapper equation and requires explicit `BisimulationAssignmentTransport` before reusing the all-keep induction; it does not claim that assignment transport follows from rule/actor equality.                                                                                                                                                                                                                                                                             |
| `Cordis.GlobalRuleRelatedDetailedReplay`                        | Adds constructor-sensitive `detailedRule` preservation to exact `RuleRelated` matches, with forward and backward assigned replay projections to the birth-erased paper package; it does not conflate L-DivertAbort with L-DivertLand.                                                                                                                                                                                                                                                                                                                                |
| `Cordis.GlobalPaperLandingReplay`                              | Concrete fixed-program Iter/Finish landing diamond assigned into a dependent trace rewrite; proves reflexive birth-erased endpoint relatedness, endpoint well-formedness, swapped detailed rules/actors, and transported assignment.                                                                                                                                                                                                                                                                                                                                                                                   |
| `Cordis.GlobalPaperFullLifecycleReplay`                        | One complete concrete `L-Begin/L-Iter/L-Finish/O-Retire/L-Leave/L-Unload` trace with actual activation assignments, explicit non-activation assignments, and an all-keep self-replay retaining endpoint, well-formedness, rules, actors, decisions, and the complete assignment; it is not peer-state lifecycle bisimulation.                                                                                                                                                                                                                                                                                          |
...
| `Cordis.GlobalPaperShiftedFullLifecycle`                       | A concrete non-reflexive six-step assigned replay: the allocator-clock-shifted peer reconstructs L-Begin/L-Iter/L-Finish/O-Retire/L-Leave/L-Unload, and the relation-aware certificate retains endpoint/WF, final relatedness, exact rule/actor ledgers, six keep decisions, and the explicit peer assignment; it is not global lifecycle bisimulation or Lemma 72/Theorem 73.                                                                                                                                                                                                                                         |
| `Cordis.GlobalPaperNonReflexiveRewrite`                        | A concrete related-endpoint adjacent rewrite over opposite-order inserts: the window endpoints are distinct and birth-erased-related but not current-`RuleRelated`, a trailing O-Retire is replayed from the related endpoint, and assignment/WF/rule/actor certificates are retained.                                                                                                                                                                                                                                                                                                                                 |
| `Cordis.GlobalPaperTraceBackwardRewrite`                       | The backward mirror of the non-reflexive related rewrite: a source suffix is replayed from the related swapped endpoint with dependent assignment transport, endpoint/WF evidence, and rule/actor permutations; the concrete witness exposes actors `[2, 1, 1]`.                                                                                                                                                                                                                                                                                                                   |
| `Cordis.GlobalPaperTraceBackwardNormalization`                 | A dependent finite chain of backward related-rewrite certificates; each link carries its transported assignment and the terminal package proves reverse-oriented birth-erased relatedness plus rule/actor permutations. It is certificate composition, not automatic normalization.                                                                                                                                                                                                                                                                                              |
| `Cordis.GlobalPaperTraceBidirectionalNormalization`             | One orientation-indexed chain API for forward and backward related-rewrite certificates; it derives source/target packages, symmetrizes backward relatedness at the wrapper boundary, and composes assignments plus rule/actor permutations.                                                                                                                                                                                                                                                                                                           |
| `Cordis.GlobalPaperTraceBidirectionalNormalizer`                | A conditional fuel-bounded normalizer over the unified forward/backward chain; an explicit authority supplies normal-form decisions, oriented links, and a decreasing Nat measure, yielding a finite related endpoint with rule/actor permutations.                                                                                                                                                                                                                                                                                              |
| `Cordis.GlobalPaperTraceBidirectionalConfluence`                | A conditional Newman-style confluence layer for the bidirectional authority; explicit selected-edge, local-join, decreasing-measure, and normal-form equivalence laws yield unique terminal `TracePackage` endpoints for successful runs.                                                                                                                                                                                                                                                                                                |
...
| `Cordis.DeepSeekHarnessEventLocalSse`                           | Carries the exact `PreparedLogRequest` endpoint through the real loopback Python/curl SSE fixture; `EventRunner` fixes the validated session index and `EventSseResult` retains the request plan, wire frames, finisher, protocol replay, and append endpoint. The header fixture checks one valid request, three frames, and `nextSeq 6 -> 7`; remote/provider/persistence/cancellation/deployed equivalence remain external.                                                                                                                                                                                                                       |
| `Cordis.DeepSeekHarnessEventSchemaLocalSse`                     | Rebuilds raw outgoing tool definitions from the dependent source-schema certificate and carries them through the same loopback SSE fixture, proving names, descriptions, raw `parameters` ASTs, model, stream flag, frames, and append endpoint. The fixture checks one raw tool, one valid request, three frames, and `nextSeq 6 -> 7`; provider validation, credentials, persistence, cancellation, and deployed equivalence remain external.                                                                                                                                                                                    |
...
| `Cordis.DeepSeekHarnessPayloadPersistenceStreamRetryCancellation` | Carries the lossless payload ledger through that pre-round cancellation prefix. The dependent result retains the restored session/payload equations beside one completed process-backed tool round and the typed timeout stop; executable projections check `8 -> 11`, eight payload rows, one round, two first-round calls, and model `0`. Blocked-read interruption, cleanup, durability, external effects, and deployed equivalence remain external. |
| `Cordis.DeepSeekHarnessPayloadPersistenceFileStreamRetryCancellation` | Repeats the payload-preserving cancellation prefix after an actual temporary-file `DurableIO.FileBackend` read. The result retains source/read-byte equality, restored session/payload equations, and the typed timeout endpoint; executable projections check equal bytes, eight payload rows, `8 -> 11`, one round, and model `0`. Fsync, stable media, crash recovery, blocked-read interruption, cleanup, external effects, and deployed equivalence remain external. |
...
| `Cordis.DeepSeekSchemaTrace`                                   | Intrinsic dependent history for schema tool rounds: each node indexes the exact starting and ending runner/model, exposes a finite list projection, proves projection length, and identifies the final round endpoint. It is a local certificate, not provider/deployment evidence.                                                                                                                                                                                                                                                                                                                                     |
| `Cordis.DeepSeekSchemaConversationLoop`                        | Fuel-bounded heterogeneous transport loop with an explicit terminal no-tool response, an intrinsic dependent round trace plus equal list projection, model/runner endpoint, and distinct exhaustion stop; retries, cancellation, persistence, and deployed semantics remain external.                                                                                                                                                                                                                                                                                                                 |
