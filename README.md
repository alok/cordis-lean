...
backward orchestration-only replay constructors, so the proved O-rules have concrete generic
trace witnesses without fabricating a lifecycle bundle. `Cordis.GlobalPaperTraceNormalization` composes a finite connected list of
...
| `Cordis.GlobalPaperTraceSimulation`                     | Birth-erased finite forward and backward trace replay with assigned-step transport, endpoint well-formedness, exact rule/actor projections, and forward/backward orchestration-only constructors; lifecycle matching remains explicit.                                                                                                                                                              |
