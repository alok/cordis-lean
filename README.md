...
| A supported current-Harness session prefix assigns fresh local call IDs and jointly validates rich appends plus intrinsic protocol events                                                                | `SessionRefinement.RefinedEvent`, `ValidatedJsonLog.projection_exact`, `validate_example`, `validate_message_example`, `validate_tool_message_example`                                                   | Restricted turn/step/tool plus text and complete assistant tool-call blocks; source surface IDs/provider metadata are retained in state, while reasoning, tool-image/multimodal blocks, replacement, and extensions fail closed. |
...
| `Cordis.SessionRefinement`                          | Stateful supported-subset Harness session decoding with fresh call-ID assignment, text plus complete assistant tool-call surface retention, and joint Session/Protocol proof-producing validation.                                                                                      |
