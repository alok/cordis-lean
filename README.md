...
Its admitted surface-message subset accepts text-only `user/message` and `assistant/message` events,
retains source IDs/provider metadata in the refinement state, and deliberately projects only text
into the smaller local session vocabulary.
...
| `Cordis.SessionRefinement`                          | Stateful supported-subset Harness session decoding with fresh call-ID assignment, text-only surface-message retention, and joint Session/Protocol proof-producing validation.                                                                                                           |
