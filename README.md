...
reader with the timed byte-prefix streamed tool round. The executable fixture restores archive
`nextSeq = 8`, accepts one real streamed tool round, then stops on caller fuel at final `nextSeq = 10`, retaining one round, model `0`, and a typed nonterminal stop. This is a local process and
memory-store certificate: durable media, crash recovery, blocked-read interruption, provider or
executable authenticity, cleanup, external effects, and deployed Harness equivalence remain
outside.
...
| `Cordis.DeepSeekHarnessPersistenceStreamBytePrefixTimeout`     | Composes memory-backed restore with a timed byte-prefix streamed tool round; the fixture checks archive `nextSeq = 8`, one accepted round, fuel exhaustion, final `nextSeq = 10`, and model `0`, with the exact restored-session equality retained. Durable media, crash recovery, blocked-read interruption, provider/process authenticity, cleanup, external effects, and deployed equivalence remain external.                                                                                                                            |
