...
source-shaped subset: text-only user messages and singleton-text tool results encode to canonical
ASTs, preserve safe integers, source-event references, replacement ranges, call IDs, and exact
`isError` values, and compose with the existing JSONL text/UTF-8 byte parser. Its successful
`decode_encode` and list-level text/byte theorems are certificates for this local subset only;
assistant surface messages, images, opaque provider metadata, and complete event-union coverage
...
| `Cordis.SessionRefinementSurfaceCodec`                         | Proof-carrying canonical AST, JSONL-text, and UTF-8-byte encoding/decoding for text-only user messages and singleton-text tool results, preserving safe integers, provenance, replacement ranges, call IDs, and `isError`; unsupported assistant surface messages, images, opaque metadata, and other event variants fail closed.                                                                                                                                                                                                            |
