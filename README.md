...
dependent executor, while provider failures preserve the conversation unchanged.
It does not execute tools or choose retry, persistence, cancellation, or failure-message
policy.
...
| A process-backed terminal outcome either preserves a provider failure or appends a finished rich assistant to `ConversationRunner`, exposing completed provider calls for the existing dependent executor | `DeepSeekOutcomeConversation.dispatchOutcome`, `executeAndDispatchOutcome`, `projectedFunctionCalls`, `fixtureFailureDispatch`, `fixtureTextDispatch`, `fixtureToolDispatch`, `fixtureMixedDispatch`, `fixtureMultiDispatch` | Typed conversation-runner handoff only; this bridge does not execute tools, authenticate provider IDs, choose failure/retry policy, persist, cancel, or prove whole-session/deployed equivalence. |
...
| `Cordis.DeepSeekOutcomeConversation`                | Process-backed terminal-outcome handoff into `ConversationRunner`: provider failures preserve the conversation, while successful rich assistants append with model/tool-count proofs and expose completed `FunctionCall` values to the existing dependent executor; tool execution and policy remain caller-controlled.                  |
