...
before request preparation. Replay-endpoint determinism is intentionally not cast or assumed;
opaque semantics, provider behavior, transport, persistence, cancellation, and complete deployed
...
| Complete current-Harness simulation composes into a certified request without losing its dependent archive/replay evidence                                                                                                                                  | `DeepSeekHarnessCompleteRequest.PreparedSimulation`, `prepare`, `prepareSimulation`, `ignorableHeaderPrepared`, `requiredOpaqueRejected`                                                                                                                                                                                                                                                                                | The request certificate is indexed by the exact normalized validator endpoint and retains the archive, keep/drop ledger, semantic replay, source/header agreement, and request-build equality. The positive ignorable-header fixture and required-opaque rejection are executable. Replay-endpoint determinism is deliberately not asserted until a separate theorem establishes it. |
