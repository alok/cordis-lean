...
abbrev State (catalog : Catalog sig) (Ambient : Type u) :=
  Cordis.GlobalRegistry.GlobalState catalog Ambient

variable {initial : State catalog Ambient}
...
structure Authority
  ...
  decreases : ∀ (package : Package values dynamics inertia initial) (notNormal : ¬ normal package),
...
structure Result
  ...
  terminal_eq : RewriteChain.terminal dynamics inertia initial source links = final
...
theorem normalize_some
  ...
  exact normalizeFuel_some_of_measure_le authority (authority.measure source) source (Nat.le_refl _)

def Result.chain
    {authority : Authority values dynamics inertia initial}
    {source : Package values dynamics inertia initial}
    (result : Result authority source) : RewriteChain values dynamics inertia initial where
  source := source
  links := result.links
...
theorem Result.terminal
    ...
    RewriteChain.terminal dynamics inertia initial source result.links = result.final :=
  result.terminal_eq
...
open Cordis.GlobalPaperTraceNormalization.Example

abbrev exampleValues := Cordis.GlobalPaperRelation.DirectedReplayExample.values
abbrev exampleDynamics := Cordis.GlobalDeletion.Positive.dynamics
abbrev exampleInertia := Cordis.GlobalDeletion.Positive.inertia
abbrev exampleInitial := Cordis.GlobalDeletion.Positive.retired

def emptyAuthority : Authority exampleValues exampleDynamics exampleInertia exampleInitial where
  measure _ := 0
  normal _ := True
...
def emptySource : Package exampleValues exampleDynamics exampleInertia exampleInitial :=
  Cordis.GlobalPaperTraceNormalization.Example.emptyPackage

def executableFuel : Nat := emptyAuthority.measure emptySource

theorem executableFuel_eq : executableFuel = 0 := rfl

theorem empty_normalizes :
    ∃ result, normalize emptyAuthority emptySource = some result :=
  normalize_some emptyAuthority emptySource
