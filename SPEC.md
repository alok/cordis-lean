...
the exact accepted prefix and projects to a cancelled phase. The fast, timeout, and race fixtures
are configured local-process evidence; task-cancellation delivery, arbitrary cleanup, fairness,
backpressure, reconnects, authenticity, and deployed asynchronous equivalence remain external.
`Cordis.DeepSeekExternalToolProcess` is the corresponding source-honest external-tool boundary:
the configured local process, stdout/stderr, exit code, parsed JSON, and typed decoded result are
retained as an observation. Only an explicit `ToolSpec.post` proof can pass that observation to
`AcceptedResult.certified` and construct a `CertifiedOutcome`; process output is not itself a
postcondition proof. Command identity, sandboxing, authentication, exactly-once effects, cleanup,
and deployed Harness equivalence remain external.
`Cordis.DeepSeekExternalToolRound` is the next typed boundary: an accepted result is indexed into
an `ExternalToolRound` whose endpoint is exactly the session append of one log-only tool call and
one cited surface tool result. The module proves the resulting sequence, message, and protocol
projection equations while leaving result rendering and provider/deployment semantics explicit.
`Cordis.DeepSeekExternalGenericRound` closes the reusable process-to-generic seam: a caller supplies
one dependent acceptance/dispatch certificate, and `observeAndDispatch` runs the configured process,
retains the complete typed observation, and returns either no accepted dispatch or an indexed
`ExternalDispatchResult`. The executable counter fixture exercises the success path through the
generic runner and rich session adapter, while a nonzero process that still emits decodable JSON is
retained but cannot inhabit `AcceptedResult`.
`Cordis.DeepSeekExternalGenericConversation` lifts that seam to a finite dependent script. A later
round is selected from the exact accepted observation and generic runner endpoint; the returned
trace retains an external-dispatch certificate for every accepted edge, while completion, fuel
exhaustion, uncertified process stops, and observation errors remain distinct. Its
`runCapturingErrors` form retains the accepted trace prefix when a later process fails to spawn or
decode, while the legacy `run` form preserves the `Except ObservationError` boundary. Heterogeneous
`ToolSpec` payloads are kept in proof-only provenance so the executable trace remains in the `IO`
universe. This is a finite local-process conversation harness, not a claim of sandboxing,
authentication, exactly-once effects, cleanup, persistence, provider obedience, or deployed Harness
equivalence.
`Cordis.DeepSeekExternalGenericSession` threads the same finite script through the rich `Session`
state. Its script index carries the exact generic runner, session, and protocol-projection proof;
accepted dispatches append the typed tool-call/tool-result pair exactly once, while fuel,
uncertified observations, and observation errors retain the current session prefix. The counter
fixtures exercise both a completed session endpoint and a later malformed-observation stop. This
remains a local finite adapter: `GenericSessionHarness.RunnerState.prepareRequestStep` now stops at
an empty-pending step after recording the exact request header and surface prompt, and its indexed
certificate proves `Session.mkRequest` is present before an external dispatch; the append
certificate preserves request reconstruction across an accepted call/result pair. Process identity,
...
`Cordis.DeepSeekSessionRequest` then consumes that indexed `Session.ModelRequest` through an explicit
`SourceAgreement`: model, system prompt, and encoded tool schemas must be carried as equalities before
the schema-indexed DeepSeek builder can produce a `PreparedRequest` and exact `RequestPlan`. Optional
DeepSeek controls remain caller policy, and parser-backed tool-schema validity, credentials, transport,
and provider behavior remain outside this pure bridge.
...
streamed jobs. `RetryProcessJob.runWithFinish` and `executeRaceWithFinish` take a caller-supplied
certified text/tool/mixed/multi finisher; `run` and `executeRace` remain the existing multi-tool
wrappers. The dependent winner retains its indexed retry trace and final endpoint; cancellation-
first and delayed-child fixtures exercise both terminal branches, including a direct text-finisher
race. Blocked-read interruption, fairness, cleanup, reconnect, and deployed retry/cancellation
...
- a checked reverse surface codec for text-only user messages, text/reasoning/complete-tool-call
  assistant messages, and singleton-text tool results, composed across canonical JSON AST, JSONL
...
| `Cordis.DeepSeekAsyncStreamHarnessTimeout`                       | Cooperative race over timer-backed byte-prefix streamed continuations; each child retains timeout configuration, dependent runner/model endpoints, and either a completed round trace or the exact timed prefix. A timeout is projected to a cancelled phase rather than a fabricated terminal response. Task-cancellation delivery, arbitrary descendant cleanup, fairness, reconnects, authenticity, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                    |
| `Cordis.DeepSeekExternalToolProcess`                             | A configured local process observation retains command configuration, stdout/stderr, exit code, parsed JSON, and a typed decoded result. `AcceptedResult.certified` requires an explicit `ToolSpec.post` proof before constructing `CertifiedOutcome`; process identity, sandboxing, authentication, exactly-once effects, cleanup, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `Cordis.DeepSeekExternalToolRound`                               | An accepted dependent result is indexed into an exact log-only tool-call plus cited surface tool-result Session append, with sequence, message, and protocol-projection theorems; rendering and provider/deployment semantics remain explicit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `Cordis.DeepSeekExternalGenericRound`                            | Reusable process-to-generic-runner handoff: `observeAndDispatch` retains the configured process observation and accepts only a caller-supplied dependent certificate, then returns an indexed `ExternalDispatchResult`; the counter fixture executes the success path through generic model/lease/record/session invariants and rejects a decodable nonzero-exit process. Process identity, sandboxing, authentication, exactly-once effects, cleanup, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                            |
| `Cordis.DeepSeekExternalGenericConversation`                     | Finite dependent scripts over proof-carrying external dispatches; accepted edges form an indexed runner trace, while completion, fuel exhaustion, uncertified stops, and captured observation errors remain distinct. `runCapturingErrors` retains an accepted prefix when a later process fails to spawn or decode, while `run` preserves the error-returning boundary. Heterogeneous process/tool payloads are retained in proof-only edge provenance so the executable result remains usable by `IO`; process identity, sandboxing, authentication, exactly-once effects, cleanup, persistence, provider obedience, and deployed Harness equivalence remain external.                                                                                                                                            |
| `Cordis.DeepSeekExternalGenericSession`                          | Session-indexed finite scripts whose exact runner/session/projection index preserves rich-session alignment across accepted external dispatches; completed dispatches append one typed tool call plus one cited tool result, and fuel/uncertified/observation-error stops retain the current rich prefix. The counter fixtures cover both a completed endpoint and a later malformed-observation stop; the generic session bridge also exposes a request-ready step whose exact header/surface state has a present `Session.ModelRequest` certificate. Process identity, sandboxing, authentication, exactly-once effects, cleanup, persistence, provider obedience, and deployed Harness equivalence remain external.                                                                                              |
| `Cordis.DeepSeekSessionRequest`                                  | Indexed bridge from `Session.ModelRequest` to the schema-generic DeepSeek request builder. `SourceAgreement` and `PreparedRequest` retain exact request equations; `executeComplete` and `executeStreamingSse` run injected complete/streaming plans, while `executeCompleteAndAppend` admits a response and appends it to the same schema-indexed `ExtensionRunner` with body, header, sequence, local-ID, tool-count, and request-reconstruction certificates. Remote transport, optional provider controls, and parser-backed schema validity remain explicit adapter obligations.                                                                                                                                                                                                                               |
| `Cordis.DeepSeekSessionRequestBytes`                             | Byte-preserving continuation of the indexed request seam: `buildCompleteBytePlan` retains the canonical UTF-8 body, and `executeCompleteBytesAndAppend` runs an injected byte transport, retains the raw response bytes plus decoded text, admits the typed response, and appends it to the same schema-indexed `ExtensionRunner`. Invalid UTF-8, HTTP, transport, and semantic response failures remain typed errors; live networking, credentials, provider behavior, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                                   |
| `Cordis.DeepSeekSessionRequestStreaming`                         | Indexed streaming continuation over a caller-supplied certified finisher: `executeStreamingAndAppend` retains the mode-indexed streaming plan, process/SSE wire frames, finished rich response, and exact `ExtensionRunner` endpoint; `executeStreamingTextAndAppend`, `executeStreamingToolAndAppend`, `executeStreamingMixedAndAppend`, and `executeStreamingMultiAndAppend` select the existing text/tool/mixed/multi finishers. Unsupported, malformed, incomplete, and provider-failure responses remain typed errors; injected process configuration and local semantics do not imply live provider or deployed Harness behavior.                                                                                                                                                                             |
| `Cordis.DeepSeekHarnessLocalSseIndexed`                          | Indexed wrapper around the real one-shot loopback HTTP/SSE fixture: `runWithFinish` accepts a caller-supplied certified finisher, while `runWithKey` selects text; each retains the local request, response, wire frames, streaming-plan equality, and exact `ExtensionRunner.appendFinished` endpoint. The executable tests cover text, tool, mixed, and multi-call finishes. Local process/HTTP evidence only; returned-body provenance, remote/TLS/credential/provider authenticity, persistence, reconnects, blocked reads, and deployed equivalence remain external.                                                                                                                                                                                                                                           |
| `Cordis.DeepSeekHarnessLocalSseIndexedLoop`                      | Two-round dependent continuation over the indexed loopback seam: `runWithFinish` carries a caller-supplied finisher through both rounds, so the first certified append becomes the exact runner index for a rebuilt second `Session.ModelRequest`; `TwoRoundResult` retains both dependent prepared requests and real SSE appends. The executable fixture checks text plus tool/mixed/multi-call variants, exact frame counts, tool allocation, and the `2 -> 4` progression. This is local one-shot process/HTTP evidence only; remote/provider authenticity, persistence, reconnects, blocked reads, and deployed equivalence remain external.                                                                                                                                                                    |
...
| `Cordis.DeepSeekAsyncStreamRetryCancellation`                    | Cooperative race over retry-aware streamed children; `runWithFinish`/`executeRaceWithFinish` accept a caller-supplied certified finisher while `run`/`executeRace` remain multi-tool wrappers. The winner retains its dependent indexed retry trace/final endpoint, and text plus multi-tool fixtures cover cancellation-first and delayed-child success-first branches. Blocked-read interruption, fairness, cleanup, reconnect, and deployed retry/cancellation equivalence remain external.                                                                                                                                                                                                                                                                                                                      |
| `Cordis.GenericSessionHarness`                                   | Reusable rich-session/request wrapper over any `GenericHarness.Config`, with exact Session-to-structural-log projection, generic call/lifecycle append transitions, `prepareRequestStep` for an indexed request-ready handoff, and `attachCompletedDispatch_modelRequest` for request reconstruction after an accepted call/result append.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `Cordis.DeepSeekCurlBytePrefixProviderAssemblyTool`              | Composes timer-driven arbitrary-byte prefix ingress with provider assembly and dependent tool/session continuation: completed streams retain exact provider, assembly, execution, and append certificates, while fuel, cancellation, and timeout retain a typed provider prefix. Byte backpressure, reconnects, process/authenticity, persistence, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `Cordis.DeepSeekHarnessCompleteSimulation`                       | Composes archive, supported-subset normalization, and occurrence-indexed replay into one `CompleteSimulation`; text, tool-message, and compaction-style replacement fixtures expose executable archive/known/keep/drop, endpoint, source-position, and source-time summaries, while malformed replacement rejects. This remains a finite source-honest subset bridge; opaque/provider/transport/persistence/cancellation and complete deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                 |
| `Cordis.DeepSeekHarnessCompleteRequest`                          | Composes a successful `CompleteSimulation` with `PreparedLogRequest`: `prepare` carries the lossless archive, keep/drop ledger, normalized semantic replay, exact validated endpoint, dependent `ModelRequest`, source/header agreement, request-build equality, and an indexed replay-endpoint equality in one result. The seven-row ignorable-header fixture prepares successfully, while a required opaque row rejects before request preparation; deployed Harness equivalence remains external.                                                                                                                                                                                        |
| `Cordis.DeepSeekHarnessEventRequest`                             | Composes a validated event-log endpoint with `Session.mkRequest` and `DeepSeekSessionRequest`: `PreparedLogRequest` retains the exact `ValidatedJsonLog`, dependent `ModelRequest`, source/header agreement, request-build equality, and protocol-projection replay equation. The header fixture succeeds and a headerless log rejects explicitly. Provider schema compatibility, credentials, transport, provider behavior, and complete deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                 |
| `Cordis.DeepSeekHarnessEventToolSchema`                          | Retains the exact current-Harness request-tool `parameters` AST alongside the compressed local header projection, validates the bounded primitive object-schema vocabulary in a dependent `ValidatedSourceToolSchema`, and attaches it to the validated `PreparedLogRequest` endpoint with raw-header and local-tool-list equalities. The `lookup` fixture succeeds and an unsupported primitive tag rejects; full JSON Schema, provider validation, model obedience, credentials, transport, and deployed Harness equivalence remain external.                                                                                                                                                                                                 |
| `Cordis.DeepSeekHarnessEventLocalSse`                            | Carries that exact `PreparedLogRequest` endpoint through the real loopback Python/curl SSE fixture. `EventRunner` fixes the validated session as the runner index, while `EventSseResult` retains the request plan, response frames, accepted finisher, protocol replay, and exact append endpoint; the header fixture checks one valid request, three frames, and `nextSeq 6 -> 7`. Remote/provider authenticity, credentials, persistence, cancellation, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                  |
| `Cordis.DeepSeekHarnessEventSchemaLocalSse`                      | Rebuilds raw outgoing `ToolDefinition`s from the dependent source-schema certificate and carries that endpoint through the same loopback SSE fixture. The indexed result proves exact names, descriptions, raw `parameters` ASTs, model, stream flag, response frames, append endpoint, and that the outgoing body is the compressed typed streaming source JSON (also tied to the successful `ChatRequest` build); the executable fixture checks one raw tool, one valid request, three frames, and `nextSeq 6 -> 7`. Provider schema validation, credentials, persistence, cancellation, and deployed Harness equivalence remain external.                                                                                                                                                       |
...
| `Cordis.DeepSeekProviderAssembler`                               | Source-shaped post-decoder `BlockAssembler` model: first-seen order, open-block fallback, first-close-wins, latest tool metadata, last usage/finish metadata, max-token tool pruning, typed unknown-block failure, and certificate-retaining validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `Cordis.DeepSeekAssemblerToolRound`                              | Converts a successful assembly to local `FunctionCall`s, executes the dependent tool list, and appends the assistant plus certified tool results to the session surface.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `Cordis.DeepSeekProviderStreamAssembly`                          | Composes the strict multi-call SSE projection with provider assembly, retaining rich-source and provider-fold certificates while rejecting replay metadata and normalizing structured failure/abort causes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `Cordis.DeepSeekProviderReplayAttachment`                        | Carries a proof-aligned rich `RawReplayEnvelope` beside the exact source-shaped provider assembly without inventing a provider replay serializer; the provider `replayState` slot remains `none`, and the wire-backed current subset has an explicit no-attachment witness.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `Cordis.DeepSeekStreamToolRound`                                 | Carries that wire-backed assembly through dependent execution and certified assistant/tool-result session append; the executable counter fixture reaches model `5` from `2`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `Cordis.DeepSeekScopedStreamToolRound`                           | Resolves each assembled call through nearest-first scopes and typed approval before dependent model-indexed execution, then appends the assistant plus certified tool results; the dual-call fixture also exercises terminal shadow and denied-approval failures.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `Cordis.DeepSeekProcessScopedConversation`                       | Finite process-backed scoped conversation indexed by a `Nat → ProcessConfig`; dependent round witnesses retain each body/model/session append, with typed terminal and fuel-exhaustion stops, and the local fixture checks both two-step completion and one-step exhaustion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `Cordis.DeepSeekProcessScopedRequestConversation`                | Request-indexed process-backed scoped conversation: each round builds a typed streaming request from the current session and indexes its prepared process/SSE/scoped result by that exact `HttpRequest`; the fixture checks distinct request bodies, terminal completion, and fuel exhaustion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `Cordis.DeepSeekProcessScopedRequestBytePrefixConversation`      | Request-indexed arbitrary-byte process conversation: each round retains the exact typed request, raw chunks, pending framing, status, strict SSE completion, scoped execution, and session append; the fixture checks two completed rounds plus a typed prefix-fuel stop.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `Cordis.DeepSeekHarnessLocalSseRequestBytePrefixConversation`    | Real loopback HTTP/SSE companion: one bounded server selects distinct SSE bodies by request index, real curl consumes arbitrary byte chunks, and the request-indexed scoped conversation retains two exact rounds plus a typed prefix-fuel stop; server request validity and exit remain inspectable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `Cordis.DeepSeekSchemaTrace`                                     | Adds an intrinsic dependent trace for successful schema tool rounds: every node is indexed by its exact starting and ending runner/model, with a finite list projection, projection-length theorem, and final-round endpoint theorem. This is a local history certificate, not provider/deployment evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `Cordis.DeepSeekSchemaConversationLoop`                          | Adds a caller-fueled loop over those transport-backed heterogeneous rounds, with the intrinsic dependent trace and equal list projection, explicit terminal no-tool witness, model/runner endpoint, and distinct fuel-exhaustion stop. Retries, cancellation, persistence, external effects, and deployed semantics remain external.                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `Cordis.DeepSeekSchemaLocalHttp`                                 | A one-shot loopback HTTP server validates a heterogeneous schema request with two declared tools and complete mode, then sends the weather/clock tool body followed by a terminal body through real curl; the dependent result retains the prepared plan, tool-round/terminal witness, request counts, server exit, final model, and runner endpoint. Remote reachability, TLS, provider authenticity, retries, cancellation, persistence, external effects, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                                              |
...
| `Cordis.DeepSeekHarnessLiveStreamProbe`                          | Credential-safe environment parsing plus a typed streaming request fed through configured curl byte chunks into the dependent byte-prefix conversation runner; the fixture retains the raw-prefix/round/endpoint witness and explicit fuel exhaustion, while credential validity, remote reachability, provider obedience, blocked-read cancellation, backpressure, retries, persistence, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                 |
| `Cordis.DeepSeekHarnessLocalHttp`                                | A one-shot loopback HTTP server validates the actual curl method, route, authorization, model, and `stream: false` body before returning typed responses; `runWithKey` retains port/request/server evidence beside the conversation endpoint, while `runCompleteAppendWithKey` exercises the same real curl boundary through the indexed accepted-response append and retains its dependent result. Remote reachability, TLS, provider authenticity, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                                                      |
...
| `Cordis.DeepSeekHarnessLocalSseRetry`                            | A two-attempt loopback SSE server returns one valid transient HTTP 503 and then a valid flushed stream; `runWithRetryAndFinish` passes only the accepted body to a caller-supplied certified text/tool/mixed/multi finisher, while `runWithRetry` selects text. The typed failure history, reused streaming plan, and append endpoint remain exact. Provider backoff, tool idempotency, arbitrary retry policy, blocked-read cancellation, credential/TLS authenticity, and deployed retry equivalence remain external.                                                                                                                                                                                                                                                                                             |
| `Cordis.DeepSeekHarnessLocalSseRetryConversation`                | Lifts the real loopback retry boundary over two dependent conversation rounds; `runTwoRoundsWithFinish` carries the caller-supplied finisher through both rounds, retaining one typed HTTP 503 per round, accepted appends, exact request/server evidence, and the rebuilt second session endpoint. Executable text/tool/mixed/multi variants cover frame counts, call allocation, and `2 -> 3` growth. Provider backoff, idempotency, arbitrary retry policy, cancellation, persistence, external effects, and deployed retry/conversation equivalence remain external.                                                                                                                                                                                                                                            |
| `Cordis.DeepSeekHarnessPersistenceFileLocalSseRetryConversation` | Composes a temporary-file `ReadCertificate` with those two dependent loopback retry rounds, retaining exact archive/session equality, `RequestProvenance` plan/body equations, one typed 503 per round, distinct rebuilt request bodies, and the executable `8 -> 10` endpoint. `withTempFile` cleanup, fsync, stable media, crash recovery, provider authenticity, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                             |
| `Cordis.DeepSeekHarnessEventFileLocalSseRetryConversation`       | Restores the supported current-Harness event JSONL fixture from a temporary file, then drives two real loopback HTTP/SSE retry rounds from that dependent event-session endpoint. The result retains byte/archive/session certificates, one typed 503 per round, both request-build/body equations, distinct rebuilt bodies, and exact `8 -> 10` growth; file durability, provider authenticity, blocked-read interruption, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                     |
| `Cordis.DeepSeekHarnessEventFileProcessSchema`                   | Restores the supported current-Harness event JSONL fixture from a temporary file, then drives the heterogeneous weather/clock schema process from that dependent endpoint. The result retains source/read bytes, archive/session equality, the streaming plan, complete body, schema step, and exact `8 -> 11` endpoint; provider obedience, credentials, durable storage, cancellation, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                        |
| `Cordis.DeepSeekHarnessEventFileLocalSseSchema`                  | Restores the supported current-Harness event JSONL fixture from a temporary file, then uses that session as the initial endpoint for two real loopback HTTP/SSE byte-prefix rounds with the heterogeneous weather/clock registry. The result retains byte/archive/session certificates, valid request counts, complete byte evidence, distinct rebuilt bodies, two-tool execution, and exact `8 -> 12` growth; the deterministic model/server, scoped approval, provider obedience, credentials, durable storage, blocked-read cancellation, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                    |
| `Cordis.DeepSeekHarnessEventFileLocalSseSchemaErrors`            | Restores the same current-event archive from a temporary file, then drives two real loopback HTTP/SSE rounds whose scoped weather/clock providers both return typed failures. The opt-in error-result policy appends certified `isError` tool results before the terminal text round, retaining valid request counts, exact failure messages, two attempts, and `8 -> 12` growth; provider obedience, credentials, durable storage, blocked-read cancellation, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                  |
| `Cordis.DeepSeekHarnessLocalSseTimeout`                          | A loopback SSE server flushes two valid lines and stalls; a real asynchronous timer interrupts the blocking curl read and retains the dependent typed prefix, while a fast fixture follows the same request path to a completed append. Arbitrary cleanup, fairness, backpressure, credential/TLS authenticity, provider-complete assembly, reconnect semantics, and deployed cancellation equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                             |
| `Cordis.DeepSeekHarnessLocalSseMultiTool`                        | A loopback SSE server validates `stream: true`, emits the existing two-call terminal stream, and routes the actual curl response through `executeConversationMultiStreamRound`; the result retains both local call IDs, dependent executions, certified tool-result appends, and the final runner endpoint. Provider-complete assembly, backpressure, cancellation, reconnects, credential/TLS authenticity, process trust, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                       |
| `Cordis.DeepSeekHarnessLocalSseMultiToolPrefix`                  | A loopback SSE server feeds each complete response line through `executeConversationMultiStreamPrefixRound`; completion reaches the dependent two-tool runner, while `LinePolicy` cancellation and read-budget exhaustion retain typed prefixes before dispatch. Byte framing, backpressure, fairness, blocked-read interruption, reconnects, provider-complete assembly, authenticity, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                           |
| `Cordis.DeepSeekHarnessLocalSseMultiToolBytePrefix`              | A loopback SSE server drives bounded curl byte chunks through `executeConversationMultiBytePrefixRound`; completion retains raw/framed byte evidence and reaches the dependent two-tool runner, while a one-read budget retains a typed raw prefix before dispatch. Byte-level backpressure, blocked-read interruption, reconnects, provider-complete assembly, authenticity, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                     |
| `Cordis.DeepSeekHarnessLocalSseRequestBytePrefixConversation`    | A bounded loopback server selects the first dual-tool and second terminal SSE body by request count; real curl byte-prefix reads feed the request-indexed scoped conversation, preserving exact request/body/session evolution and separate complete/prefix-fuel stops. Network/authentication, executable trust, backpressure, reconnects, persistence, external effects, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                        |
| `Cordis.DeepSeekHarnessLocalSseProviderAssemblyTool`             | A loopback SSE server validates a typed `stream: true` request, emits the incremental provider tool body, and routes real curl lines through provider assembly, dependent execution, and certified session append; the fixture reaches model `5` with one assembled call. Provider obedience, byte/backpressure semantics, blocked-read cancellation, reconnects, authenticity, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                   |
| `DeepSeekHarnessLocalSseBytePrefixProviderAssemblyTool`          | `Cordis.DeepSeekHarnessLocalSseBytePrefixProviderAssemblyTool`: a loopback SSE server validates the same typed request, real curl consumes one-byte chunks, and the byte-prefix state feeds provider assembly, dependent execution, and certified local evidence; the fixture reaches model `5`. Provider obedience, timeout fairness, reconnects, authenticity, and deployed Harness equivalence remain external.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `Cordis.DeepSeekStreamHarnessRetry`                              | Checked complete-body streamed retry: `executeWithRetryAndFinish` accepts a caller-supplied text/tool/mixed/multi finisher, retries process and transient-HTTP `SessionClientError`s under an explicit policy, retains the exact ordered failure history with a bound, and `executeConversationStreamRound` carries the selected finisher into the dependent assistant/tool continuation; the existing multi-tool functions remain wrappers. Stream framing, semantic response, and tool failures remain terminal; backoff, idempotency, cancellation, persistence, and deployed retry equivalence remain external.                                                                                                                                                                                                 |
| `Cordis.DeepSeekStreamHarnessRetryConversation`                  | Process-backed retry-aware streamed conversation: `runWithFinish` carries a caller-supplied finisher through indexed fuel-bounded rounds, whose heads retain each complete streamed body, retry history, dependent assistant/tool endpoint, and exact runner/model continuation; `run` remains the multi-tool wrapper. Completion, exhaustion, and bounded transient-HTTP failure remain distinct. Executable direct retry fixtures cover text/tool/mixed/multi raw semantic counts, while the dependent loop covers text and the admitted counter tool. Backoff, idempotency, blocked-read cancellation, persistence, reconnects, and deployed retry equivalence remain external.                                                                                                                                  |
| `Cordis.DeepSeekStreamHarnessRetryCancellation`                  | Boundary-safe pre-round cancellation over the indexed retry-aware streamed conversation: `runWithFinish` carries a caller-supplied text/tool/mixed/multi finisher, while `run` remains the multi-tool wrapper; cancellation retains the exact accepted retry-aware trace prefix, unchanged runner/model endpoint, round/reason decision, and per-round retry history, with completed and fuel-exhausted stops distinct. It does not interrupt in-flight process/HTTP/stream/tool IO or claim cleanup, reconnect, or deployed Harness cancellation equivalence.                                                                                                                                                                                                                                                      |
...
1. **External adapters.** `Cordis.DeepSeekApi` supplies the checked
   ...
   `Cordis.DeepSeekProviderAssembler` models the canonical post-decoder
   `BlockAssembler` state with first-seen order, open-block fallback, first-close-wins,
   latest tool metadata, last metadata wins, max-token tool pruning, and typed
   unknown-block rejection; wire decoding and deployed equivalence remain external.
   `Cordis.DeepSeekAssemblerToolRound` then projects a successful assembly into the
   existing dependent `FunctionCall`/`ExecutedTool` path and appends both assistant and
   certified tool-result messages. Provider-ID authenticity, external effects, transport,
   persistence, and deployed equivalence remain caller obligations.
   `Cordis.DeepSeekProviderStreamAssembly` composes the strict multi-call SSE projection
   with the same provider assembler and retains both source and fold certificates. It
   rejects replay metadata and normalizes structured failure/abort causes; transport,
   image/tool-result schemas, and deployed equivalence remain external.
   `Cordis.DeepSeekStreamToolRound` carries that accepted wire-backed result through the
   dependent execution/session path, with the local counter fixture reaching `5` from `2`.
   `Cordis.DeepSeekScopedStreamToolRound` applies the scoped registry and approval policy to
   each call in that assembled body, retaining a model-indexed `ScopedExecutionTrace` and
   converting it into the existing certified assistant/tool-result append. The fixture executes
   two dependent calls and checks terminal shadowing plus denied explicit approval; scope
   construction, authenticated policy, external effects, persistence, cancellation, and
   deployed equivalence remain outside.
   `Cordis.DeepSeekProcessStreamToolRound` places the configured process adapter in front of
   that exact round. Its local fixture retains the HTTP/SSE process certificate before reusing
   provider assembly, dependent execution, and session append; network reachability, process
   trust, incremental delivery, cancellation, persistence, and deployed equivalence remain
   external.
   `Cordis.DeepSeekProcessScopedStreamToolRound` is the corresponding process-backed scoped
   continuation. `executeWith` retains process/status and strict SSE evidence, then carries the
   exact body through scoped approval, dependent execution, and session append. The deterministic
   dual-call fixture checks three accepted frames and the final three-message endpoint; network,
   authentication, incremental delivery, cancellation, persistence, external effects, and deployed
   equivalence remain external.
   `Cordis.DeepSeekProcessScopedConversation` lifts this process-backed scoped round to a finite
   dependent conversation. The caller supplies a process family `Nat → ProcessConfig`, so the
   second fixture round can be a distinct terminal body; each accepted round retains its typed
   body/model/session witness. A no-call response constructs a typed completion stop, while fuel
   exhaustion remains a separate stop. The local fixture checks both two-step completion and
   one-step exhaustion; retries, cancellation, persistence, external effects, and deployed
   equivalence remain outside.
   `Cordis.DeepSeekProcessScopedRequestConversation` closes the request-provenance seam around
   that loop. Each round builds a typed `stream = true` request from the current session, passes
   that exact request to the local process/SSE adapter, and retains the resulting scoped round
   under the same `HttpRequest` index. The fixture proves that successive request bodies differ
   as the session grows and retains both terminal completion and fuel exhaustion; live network,
   credentials, retries, cancellation, persistence, external effects, and deployed equivalence
   remain outside.
   `Cordis.DeepSeekProcessScopedRequestBytePrefixConversation` carries that same request index
   through arbitrary-byte process ingress. Completed witnesses retain raw chunks, pending framing,
   status, strict SSE completion, scoped execution, and exact session append; the fixture proves
   two completed rounds with multi-chunk input and a typed prefix-fuel stop. It remains bounded
   local-process evidence, not live network behavior, executable trust, blocked-read cancellation,
   backpressure, reconnects, retries, persistence, external effects, or deployed Harness
   equivalence.
