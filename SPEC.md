...
| `Cordis.DeepSeekHarnessCompleteRequest`                          | Composes a successful `CompleteSimulation` with `PreparedLogRequest`: `prepare` carries the lossless archive, keep/drop ledger, normalized semantic replay, exact validated endpoint, dependent `ModelRequest`, source/header agreement, and request-build equality in one result. The seven-row ignorable-header fixture prepares successfully, while a required opaque row rejects before request preparation. Replay/validator endpoint equality remains a future determinism obligation rather than an assumed cast; deployed Harness equivalence remains external.                                                                                                                                                                                                                                    |
