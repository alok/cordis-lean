...
| `Cordis.DeepSeekHarnessLocalSseIndexed`                          | Indexed wrapper around the real one-shot loopback HTTP/SSE fixture: `runWithKey` retains the local request, response, wire frames, streaming-plan equality, and exact `ExtensionRunner.appendFinished` endpoint; `Example.run` checks one valid request, three frames, and the `2 -> 3` session progression. The fixture is local process/HTTP evidence only and does not assert returned-body provenance, remote/TLS/credential/provider authenticity, persistence, reconnects, blocked reads, or deployed equivalence.                                                                                                                                                                                                                                                                                            |
| `Cordis.DeepSeekHarnessLocalSseIndexedLoop`                      | Two-round dependent continuation over the indexed loopback seam: the first certified append becomes the exact runner index for a rebuilt second `Session.ModelRequest`, whose prepared request and second real SSE append are retained in one `TwoRoundResult`; `Example.run` checks two valid requests, six frames, and the `2 -> 4` progression. This is local one-shot process/HTTP evidence only; remote/provider authenticity, persistence, reconnects, blocked reads, and deployed equivalence remain external.                                                                                                                                                                                                                                                                                               |
