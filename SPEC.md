...
| `Cordis.DeepSeekSessionRequest`                                  | Indexed bridge from `Session.ModelRequest` to the schema-generic DeepSeek request builder. `SourceAgreement` retains exact model/system/tool-schema equalities; `PreparedRequest` retains the successful `ChatRequest` equation and can construct an exact raw `RequestPlan` plus type-indexed complete/streaming plans whose `stream` flags are proved. `executeComplete` runs a complete plan through an injected transport and retains the API's dependent response certificate; remote transport, optional provider controls, and parser-backed schema validity remain explicit adapter obligations.                                                                                                                                                                                                            |
