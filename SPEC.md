...
| `Cordis.DeepSeekHarnessEventSchemaLocalSse`                      | Rebuilds raw outgoing `ToolDefinition`s from the dependent source-schema certificate and carries that endpoint through the same loopback SSE fixture. The indexed result proves exact names, descriptions, raw `parameters` ASTs, model, stream flag, response frames, and append endpoint; the executable fixture checks one raw tool, one valid request, three frames, and `nextSeq 6 -> 7`. Provider schema validation, credentials, persistence, cancellation, and deployed Harness equivalence remain external.                                                                                                                                                       |
