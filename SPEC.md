...
| `Cordis.DeepSeekHarnessEventToolSchema`                          | Retains the exact current-Harness request-tool `parameters` AST alongside the compressed local header projection, then validates the bounded primitive object-schema vocabulary in a dependent `ValidatedSourceToolSchema`. The `lookup` fixture succeeds and an unsupported primitive tag rejects; full JSON Schema, provider validation, model obedience, credentials, transport, and deployed Harness equivalence remain external.                                                                                                                                                                                                 |
