...
exhaustion, and uncertified process stops remain distinct. Heterogeneous `ToolSpec` payloads are
kept in proof-only provenance so the executable trace remains in the `IO` universe. This is a
finite local-process conversation harness, not a claim of sandboxing, authentication, exactly-once
effects, cleanup, persistence, provider obedience, or deployed Harness equivalence.
